...
public class GestureSettings extends PreferenceFragment implements
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        if (preference == mSwipeUpSwitch) {
            ...
            ZenUtils.writeValue(getFile(), mSwipeUpSwitch.isChecked() ? "1" : "0");
            return true;
        }
    ...
    public static String getFile() {
        if (ZenUtils.fileWritable(SWIPEUP_PATH)) {
            return SWIPEUP_PATH;
        }
        return null;
    }
    ...
    private void setGestureEnabled(int id, boolean enabled) {
        ...
        int gestureMode = Integer.parseInt(ZenUtils.readLine(GESTURE_CONTROL_PATH));
    ...
    private boolean isGestureSupported(String key) {
        return ZenUtils.fileWritable(getGestureFile(key));
    }

    private void setGestureEnabledGoogle(String key, boolean enabled) {
        ZenUtils.writeValue(getGestureFile(key), enabled ? "1" : "0");
    }
