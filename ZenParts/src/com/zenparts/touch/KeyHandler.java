...
public class KeyHandler implements AlternativeDeviceKeyHandler {
    ...
    private SensorEventListener mProximitySensor = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (mUseProxiCheck) {
                if (ZenUtils.fileWritable(GOODIX_CONTROL_PATH)) {
                    ZenUtils.writeValue(GOODIX_CONTROL_PATH, mProxyIsNear ? "1" : "0");
                }
            }
    ...
    private void enableGoodix() {
        if (ZenUtils.fileWritable(GOODIX_CONTROL_PATH)) {
            ZenUtils.writeValue(GOODIX_CONTROL_PATH, "0");
        }
    }

    private void updateDoubleTapToWake() {
        Log.i(TAG, "udateDoubleTapToWake " + mDoubleTapToWake);
        if (ZenUtils.fileWritable(DT2W_CONTROL_PATH)) {
            ZenUtils.writeValue(DT2W_CONTROL_PATH, mDoubleTapToWake ? "1" : "0");
        }
    }
    ...
    private boolean launchSpecialActions(String value) {
        if (value.equals(AppSelectListPreference.TORCH_ENTRY)) {
            ...
            doHapticFeedback();
            Utils.sendKeycode(KeyEvent.KEYCODE_PAGE_DOWN);
            return true;
        } else if (value.equals(AppSelectListPreference.BROWSE_SCROLL_UP_ENTRY)) {
            doHapticFeedback();
            Utils.sendKeycode(KeyEvent.KEYCODE_PAGE_UP);
            return true;
